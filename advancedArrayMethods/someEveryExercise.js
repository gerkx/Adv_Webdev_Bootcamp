...
function hasOddNumber(arr){
    return arr.some(value => {return value % 2 > 0;});
}
...
function hasAZero(num){
    return num.toString().split("").some(value => {return value == 0;});
}
...
function hasOnlyOddNumbers(arr){
    return arr.every(value => {return value % 2 != 0;});
}
...
function hasCertainKey(arr, key){
    return arr.every(obj => {return obj[key];});
}
